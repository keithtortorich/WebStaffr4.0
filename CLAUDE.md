...
- Brand/naming governance for all customer-facing and marketing surfaces (site copy, investor materials) is set by the founder's brand source docs, not by this repo. **Resolved 2026-07-30:** founder confirmed the Google Drive "NetBuild.Pro" folder's docs (NETBUILD_GOVERNANCE.md, NetBuild_Training_Manual_real, and siblings) are the newest canonical sources. NETBUILD_GOVERNANCE.md is explicit and unconditional: no em-dashes anywhere in NetBuild.Pro copy, internal or external, no exceptions. That supersedes the Brand Principles Handbook PDF's looser (no-ban) treatment wherever the two conflict. See `docs/DECISIONS.md` ADR-020.
