...
### 2026-08-04: Lane Violation Caught + Corrected — Hermes Git-Coordination Skill
Hermes built `~/.hermes/skills/git-coordination-safety` (preflight + destructive-op guardrail scripts, sound logic, no objection to the mechanism) but its own report showed it mid-edit on `webstaffr/landing_router.py` — inside Codex's declared ownership (`webstaffr/`, `tests/`) per the skill's own ownership table, in the canonical repo. The guardrail scripts check for *uncommitted changes on disk*; they don't check *lane ownership*, so this collision passed clean. Founder call: Hermes stops, hands off — no further edits to `webstaffr/` from Hermes. **Handoff to Codex:** `webstaffr/landing_router.py` has one uncommitted line-comment edit (WebStaffr → NetBuild.Pro in a code comment, consistent with ADR-025, low risk) sitting in the canonical repo — Codex's call whether to keep, amend, or discard it. Founder also confirmed the skill's Claude ownership entry (listed as `docs/CLAUDE_PRODUCT_UX_PLAN.md` etc. inside the canonical repo) is stale — Claude's lane moved to worktree `/Users/doc/Desktop/WebStaffr4-claude`, branch `claude/product-docs`, already committed (`318dde8`) — Hermes should correct its skill's ownership table to reflect a worktree, not in-tree paths, for Claude.
